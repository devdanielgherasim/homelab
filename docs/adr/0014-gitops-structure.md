...
- **Argo CD manages itself, carefully.** It is installed once by the platform stage
  (ADR-0016) from the values file in Git; an Application then adopts that installation
  from the same file. It has no cascade-delete finalizer and `prune: false`, and the chart version
...
- **Three AppProjects.** `bootstrap` (Argo CD and the root; the only project allowed
  ...
  access to Argo CD.
- **Cilium stays outside Argo CD.** The CNI is installed by the platform
  stage from `kubernetes/bootstrap/cilium/`; a network layer reconciled by a controller that
...
- **Public repository, no secrets.** Argo CD reads the repository over HTTPS without
  ...
  password) is generated by the platform stage (ADR-0016) into the cluster and the
  encrypted OpenTofu state, and never appears in Git.
