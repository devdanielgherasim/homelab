...
| `cp01`, `worker01`, `worker02` | DEPLOYED | Cloud-init done, hardened, joined to the cluster. Since 2026-09-20 `worker02` runs on the second Proxmox node (below) with 5.5 GiB and 3 vCPU, and `cp01` has 6 GiB. |
| Second Proxmox node `pve02` (standalone, ADR-0020) | DEPLOYED, verified; ADR Proposed | Bootstrapped stage by stage on 2026-09-20 (network check, OpenTofu role and pool, SSH key, permissive firewall rules, CA bundle, VM template 9000); `firewall_enforce` and `ssh_lockdown` not run on either node, and the firewall role writes `policy_in`/`policy_out` to `host.fw` where Proxmox rejects them. `worker02` moved to it with the same name, VMID and address; the old VM is still on `pve01`, stopped and protected, until it is destroyed on purpose. |
| Adding nodes and workers with one command | IMPLEMENTED, checked offline | `scripts/proxmox-node.sh`, `scripts/worker.sh`, `proxmox_nodes` (three provider slots), a temporary OpenTofu session per Proxmox host. The session was run for real with `bootstrap.sh vms --plan-only` on both hosts; the two scripts have not been run end to end on real hosts yet ([runbook](docs/runbooks/add-node-and-worker.md)). |
...
| Platform bootstrap (OpenTofu) | DEPLOYED, verified, PROVEN by a rebuild from Git | ADR-0016 (Accepted), `tofu/environments/platform/`. Second stage after Ansible: Cilium and Argo CD as `helm_release`, AppProjects and root Application from Git, LoadBalancer pool from a private variable, `monitoring` namespace and generated `grafana-admin` Secret. State is local, gitignored and encrypted (OpenTofu state encryption, passphrase in `TF_ENCRYPTION`). The running cluster was adopted on 2026-09-20 with imports (10 imported, 2 added, 10 changed, 0 destroyed), the second plan is empty. **Rebuilt from Git on 2026-09-20** ([runbook](docs/runbooks/rebuild-cluster.md)): the three cluster VMs were replaced and the chain (Ansible stages, then the platform stage, then Argo CD) brought back a cluster identical to the baseline: 3 nodes, 12 Applications `Synced/Healthy`, the same pods and Helm charts, Gateway 90/10 through the LoadBalancer, STRICT mTLS enforced, 20/20 Prometheus targets `up`, an empty second plan; about 27 minutes of downtime. Not proven: bare-metal Proxmox, restoring a live cluster from an etcd snapshot, the `vms` stage of `scripts/bootstrap.sh` end to end. The state passphrase and the state file need a backup outside this machine. |
...
| Metrics API (metrics-server) and kubelet serving certificates | DEPLOYED, verified | ADR-0018, `kubernetes/platform/system/`, role `kubelet_server_tls`. The kubelets get their serving certificates from the cluster CA (`serverTLSBootstrap`, approved by kubelet-csr-approver 1.2.15); metrics-server 0.9.0 (chart 3.14.0) verifies them, without `--kubelet-insecure-tls`, so `kubectl top` works and the horizontal pod autoscaler has its input. Prometheus scrapes the kubelets with verification on; no ServiceMonitor or PodMonitor skips it. Rolled out node by node on 2026-09-20 with no pod restarted; 9 MiB (approver) and 20 MiB (metrics-server) in use. Not done: the API server's own `--kubelet-certificate-authority` (CIS 1.2.5, restarts the API server). `providerIpPrefixes` is unset so that the node network stays out of Git, and DNS resolution is bypassed (node names are not in DNS). The certificate path of a rebuild from zero is reasoned about, not rebuilt. |
| Network policies (default-deny) | DEPLOYED, enforced, verified | ADR-0017, `kubernetes/platform/policies/`. 23 `CiliumNetworkPolicy` objects in `argocd`, `monitoring`, `istio-system`, `mesh-demo` and `demo` (a default-deny plus one policy per workload, written from 12,268 Hubble flows, no addresses in Git). Rolled out through Cilium audit mode: four paths found and explained, checked with a negative control, then enforced on 2026-09-20. After enforcement: 12 Applications `Synced/Healthy`, 27 of 27 Prometheus targets `up`, Gateway 40 of 40, Argo CD sync with its hook Job and a new meshed pod succeeded, and a test pod was refused for the internet and for another namespace. Not covered: `kube-system`, `cilium-secrets`, `default`. Argo CD may reach the internet on 443 (repository access), not by name. |
| Prometheus / Grafana (metrics) | DEPLOYED, verified | `kubernetes/platform/observability/`, kube-prometheus-stack 91.4.1 by Argo CD, trimmed (Alertmanager and control-plane scraping off, 3 days on an `emptyDir` because there is no StorageClass, Grafana login required, credentials generated by the platform stage). 27 of 27 Prometheus targets `up`, including Cilium agents, the Cilium operator and Hubble flow metrics (PodMonitors in Git). Dashboards: "Homelab / Capacity" (requests against allocatable, input for the later autoscaling), the kube-prometheus-stack set and six Cilium/Hubble ones. About 770 MiB of memory requests; measured working set: Prometheus 334 MiB of 700 MiB (81,000 series), Grafana pod about 410 MiB. Verified on 2026-09-20 after a rebuild from Git. |
| Loki / Tempo (logs, traces) | PLANNED | Deferred: logs need a storage decision first (no StorageClass) and the workers have little RAM left. |
...
| Component versions vs Kubernetes | Inside the upstream-tested matrix | Cilium 1.20.2 and Argo CD 3.5 list Kubernetes 1.33-1.36 and Istio 1.31 lists 1.32-1.36; metrics-server 0.9.0 and kubelet-csr-approver 1.2.15 do not state 1.37 either; the cluster runs 1.37.0 |
