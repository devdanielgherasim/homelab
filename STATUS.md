...
Last updated: 2026-09-16 (guest hardening deployed and verified).
...
| `cp01` | DEPLOYED | VMID 102, `192.168.1.51`. Cloud-init `status: done`, hardened. Kubernetes not yet installed on it. |
| `worker01` / `worker02` | DEPLOYED | VMIDs 103/104, `192.168.1.52`/`.53`. Cloud-init `status: done` on both, hardened. Kubernetes not yet installed. |
...
| Ansible node configuration | PLANNED | `ansible/` has `proxmox_bootstrap`, `proxmox_template`, `guest_hardening` (all deployed); containerd/kubeadm-prerequisite role not started. |
...
1. containerd + kubeadm prerequisites on `cp01`/`worker01`/`worker02`,
